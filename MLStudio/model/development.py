...
class ModelBuilder(BaseEstimator):
    ...
    def _evaluate_model_generalization(self):
        ...
        cv_results = cross_validate(estimator=self._gridsearch_object,
                                      ...
                                      n_jobs=self.n_jobs)
        self.performance_estimate_ = cv_results
        self.performance_estimate_.pop('estimator', None)
        self.gridsearches_ = cv_results.get('estimator')

    def _select_best_model(self):
        """Select best model using inner_cv on entire training set."""
        gscv = GridSearchCV(estimator=self._pipeline,
                         param_grid=self._parameters,
                         scoring=self.scoring,
                         n_jobs=self.n_jobs,
                         cv=self.inner_cv,
                         refit=self.refit)
        gscv.fit(self.X_train_, self.y_train_)
        self.cv_results_ = gscv.cv_results_

    def _evaluate_best_model(self):
        """Evaluate best model on the held-out validation/test set."""
        estimator = self.cv_results_.get('best_estimator_')
        estimator.fit(self.X_train_, self.y_train_)
        self.final_test_score_ = estimator.score(self.X_test_, self.y_test_)

    def _build_final_model(self):
        """Refits model on all available data.""" 
        estimator = self.cv_results_.get('best_estimator_')
        self.final_model_ = estimator.fit(self.X_, self.y_)

    def _compute_sample_sizes(self):
        self.input_size_ = self.X_.shape[0]
        self.train_set_size_ = self.X_train_.shape[0]
        self.test_set_size_ = self.X_test_.shape[0]        

    def _compute_hypothesis_space_cardinality(self):
        """Computes the size of the hypothesis space."""
        return len(ParameterGrid(self._parameters))

    def _compute_pac_sample_complexity(self):
        """Computes sample complexity"""
        size_H = self._compute_hypothesis_space_cardinality()
         self.pac_sample_complexity_ = 1/self.pac_risk_approximation_tol_ * \
             (np.log(size_H)+np.log(1/self.pac_failure_prob_))

    def _compute_pac_generalization_bound(self):
        """Computes upper bound on risk."""
        self.risk_upper_bound_ = 1 / self.train_set_size_ * \
            (np.log(self._compute_hypothesis_space_cardinality) + \
                np.log(1/self.pac_failure_prob))

    def _wrap_up(self):
        """Compute remaining attributes."""
        self._compute_sample_sizes()
        self._compute_pac_sample_complexity()
        self._compute_pac_generalization_bound()
