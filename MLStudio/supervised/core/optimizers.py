...
class Optimizer(ABC, BaseEstimator):
    ...
    def update(self, gradient, theta, learning_rate, **kwargs):
        ...
        theta : array-like shape (n_features,) or (n_features, n_classes)
        ...
        **kwargs : dict
            Arbitrary arguments required by gradient

        Returns
        -------
        theta : the updated parameters
        grad : the gradient computed at theta
        """
        pass
...
class Momentum(Optimizer):
    ...
    def __call__(self, gradient, theta, learning_rate, **kwargs):             
        grad = gradient(theta)
...
class RMSprop(Optimizer):
    ...
    def __call__(self, gradient, theta, learning_rate, **kwargs):                
        ...
        self.avg_sqr_gradient = self.gamma * self.avg_sqr_gradient + \
            0.1 * np.square(grad)
        rms_grad = np.sqrt(np.square(self.avg_sqr_gradient) + self.epsilon)
        theta = theta - (self.learning_rate / rms_grad) * grad
...
class AdaMax(Optimizer):
    ...
    def __init__(self, learning_rate=0.002, beta_one=0.9, beta_two=0.999):
        self.learning_rate = learning_rate
        self.beta_one = beta_one
        self.beta_two = beta_two        
        self.t = 0
...
class Nadam(Optimizer):
    ...
    def __init__(self, learning_rate, beta_one=0.9, epsilon=10e-8):
        self.learning_rate = learning_rate
        self.beta_one = beta_one        
        self.epsilon = epsilon
        self.t = 0
    ...
    def __call__(self, gradient, theta, learning_rate, **kwargs):    
        ...
        # Nadam update
        theta = theta - (self.learning_rate / (np.sqrt(v_hat) + self.epsilon)) * \
